...
test("AIOS creates isolated channel jobs", () => {
  const aios = createAIOS();
  aios.registerChannel({ id: "cashvolt", name: "CashVolt" });
  aios.registerChannel({ id: "channel-2", name: "Channel 2" });

  const first = aios.createJob("cashvolt", { topic: "money" });
  const second = aios.createJob("channel-2", { topic: "science" });

  assert.equal(first.channelId, "cashvolt");
  assert.equal(second.channelId, "channel-2");
  assert.equal(aios.persistence.listJobs("cashvolt").length, 1);
  assert.equal(aios.persistence.listJobs("channel-2").length, 1);
});
